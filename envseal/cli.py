...
app = typer.Typer(
    name="envseal",
    help="Manage local developer secrets and optional encrypted .env vaults",
...
def update(
    ...
    for item in selected:
        selected_change: ChangeInfo = item.data
        console.print(
            f"📁 Checking [cyan]{selected_change.repo_name}/{selected_change.env_name}[/cyan]..."
        )
        ...
        try:
            ...
            local_normalized = dotenv_io.normalize(selected_change.env_file.filepath)
            vault_decrypted = sops.decrypt(selected_change.vault_path)
            ...
            if not (current_diff.added or current_diff.modified or current_diff.removed):
                console.print(
                    f"  ⊘ [dim]{selected_change.env_name}.env - no changes detected, skipped[/dim]"
                )
                skipped_count += 1
                continue

            # Ensure vault directory exists
            selected_change.vault_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".env", delete=False) as tmp:
                ...
                # Parse and write normalized
                data = dotenv_io.parse(selected_change.env_file.filepath)
                dotenv_io.write(tmp_path, data)

                # Encrypt
                sops.encrypt(tmp_path, selected_change.vault_path)
                tmp_path.unlink()

            console.print(f"  ✓ [green]{selected_change.env_name}.env updated[/green]")
            updated_count += 1
