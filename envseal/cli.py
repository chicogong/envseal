...
def _render_report_html(overview: dict[str, list[tuple[str, list[str]]]]) -> str:
    ...
    for repo_name in sorted(overview):
        ...
        rows.append(f'<h2>{_html.escape(repo_name)} <span class="count">{total} keys</span></h2>')
    ...
    body = "\n".join(rows) if rows else "<p>No secrets in the vault yet.</p>"
...
<p class="note">Key names only - no secret values appear in this file. Safe to share.</p>
__BODY__
<p class="note">Generated by <code>envseal report</code>.</p>
</body></html>
"""
